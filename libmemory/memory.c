...
void device_free(struct simulation *sim,struct device *in)
{
	...
	free_zx_gdouble(in,in->Fi0_top);
	free_zx_gdouble(in,in->Vl);
	...
	free_3d_gdouble(in,in->muion);
	free_3d_gdouble(in,in->Nion);
	free_3d_gdouble(in,in->Nion_last);
	free_3d_gdouble(in,in->Dn);
	free_3d_gdouble(in,in->Dp);
...
void device_get_memory(struct simulation *sim,struct device *in)
{
	...
	malloc_zx_gdouble(in,&(in->Fi0_top));
	malloc_zx_gdouble(in,&(in->Vl));
	malloc_zx_gdouble(in,&(in->Vr));
	...
	malloc_3d_gdouble(in,&(in->muion));

	malloc_3d_gdouble(in,&(in->Nion));

	malloc_3d_gdouble(in,&(in->Nion_last));
