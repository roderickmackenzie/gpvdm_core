...
buffer_init(&buf);
	...
	zxy_malloc_gdouble(dim, &temp_3d);
	buffer_malloc(&buf);
	...
	get_meter_dim(buf.x_units,&mul_x,dim->xmesh[dim->xmeshpoints-1]);
	get_meter_dim(buf.y_units,&mul_y,dim->ymesh[dim->ymeshpoints-1]);
	get_meter_dim(buf.z_units,&mul_z,dim->zmesh[dim->zmeshpoints-1]);
	buf.y_mul=mul_y;
	buf.x_mul=mul_x;
	buf.z_mul=mul_z;
	...
	mem_set_zx_gdouble_from_zx_gdouble(dim, temp_top, in->Vl);
	mem_add_zx_gdouble_from_zx_gdouble(dim, temp_top, in->Vapplied_l);

	mem_set_zx_gdouble_from_zx_gdouble(dim, temp_btm, in->Vr);
	mem_add_zx_gdouble_from_zx_gdouble(dim, temp_btm, in->Vapplied_r);
	...
	three_d_set_gdouble(dim, temp_3d, 0.0);
	...
	//getchar();
	buffer_add_3d_device_data_including_boundaries(sim,&buf,in,  in->Jn,in->Jnleft,in->Jnright);
	buffer_dump_path(sim,out_dir,name,&buf);
	buffer_free(&buf);
	...
	buf.z=dim->zmeshpoints;
	buffer_add_info(sim,&buf);
	buffer_add_3d_device_data_including_boundaries(sim,&buf,in,  in->Jp,in->Jpleft,in->Jpright);
	buffer_dump_path(sim,out_dir,name,&buf);
	buffer_free(&buf);
	...
	//sprintf(temp,"%Le %Le\n",dim->ymesh[dim->ymeshpoints-1]-dim->ymesh[1]/2,get_J_right(in));
	buffer_dump_path(sim,out_dir,name,&buf);
	buffer_free(&buf);
	...
	strcpy(buf.y_label,_("y-position"));
	strcpy(buf.z_label,_("z-position"));
	strcpy(buf.data_label,_("Net recombination rate"));
	strcpy(buf.data_units,"m^{-3} s^{-1}");
	strcpy(buf.section_one,_("1D position space output"));
	strcpy(buf.section_two,_("Recombination"));
	buf.logscale_x=0;
	...
	buf.z=dim->zmeshpoints;
	buffer_add_info(sim,&buf);
	buffer_add_3d_data(sim,&buf,dim,  in->Rnet);
	buffer_dump_path(sim,out_dir,name,&buf);
	buffer_free(&buf);
...
*/
	...
	buf.z=dim->zmeshpoints;
	buffer_add_info(sim,&buf);
	buffer_add_2d_device_data_int(sim,&buf,in,in->passivate_l);
	buffer_dump_path(sim,out_dir,name,&buf);
	buffer_free(&buf);
	...
	buf.z=dim->zmeshpoints;
	buffer_add_info(sim,&buf);
	buffer_add_2d_device_data_int(sim,&buf,in,in->passivate_r);
	buffer_dump_path(sim,out_dir,name,&buf);
	buffer_free(&buf);
