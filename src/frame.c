...
/** @file frame.c
@brief Used to make 2D maps, not used any more due to python fron end.
*/
...
if (in->data[xpos][ypos]<data) in->data[xpos][ypos]=data;
}
...
for (x=0;x<in->xpoints;x++)
{
	...
	for (y=in->ypoints-1;y>=0;y--)
	{
			...
			if (ret!=0.0)
			{
				if (count<2)
				{
					if (fabs(l.min_y-in->y[y])>fabs(l.max_y-in->y[y]))
					{
					set_vec(&v, in->x_fake[x],in->y_fake[y], 0.0);
					list_add_no_rep(&l,&v);
					}
				}else
				{
...
for (x=0;x<in->xpoints;x++)
{
	for (y=0;y<in->ypoints;y++)
	{
		...
		{
			if ((in->data[x-1][y]==1)&&(in->data[x+1][y]==1)) in->data[x][y]=1.0;
		}
